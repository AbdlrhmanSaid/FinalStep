...
export default function HowItWorksClient() {
  ...
  const content = {
    ...
    features: [
      ...
      },
      {
        ...
        title: isRTL
          ? "نظام التقييم الذكي (Smart Evaluate)"
          : "Smart Evaluate System",
      ...
      },
      {
        ...
        title: isRTL
          ? "التقارير المتقدمة والطباعة"
          : "Advanced Reports & Printing",
  ...
  return (
    <div
      className={`min-h-screen bg-gray-50 dark:bg-gray-900 ${isRTL ? "rtl" : "ltr"} pb-24`}
    >
      {/* Hero Section */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          {content.features.map((feature, idx) => (
            ...
            >
              ...
              <div
                className={`absolute top-0 ${isRTL ? "left-0" : "right-0"} w-32 h-32 ${feature.color} rounded-full blur-3xl opacity-50 -translate-y-1/2 ${isRTL ? "-translate-x-1/2" : "translate-x-1/2"} group-hover:opacity-70 transition-opacity`}
              ></div>

              <div className="relative z-10">
                <div
                  className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 shadow-sm border ${feature.borderColor} ${feature.color} transform group-hover:scale-110 transition-transform duration-300`}
                >
                  {feature.icon}
                </div>
                ...
                <ul className="space-y-4">
                  {feature.items.map((item, i) => (
                    <li key={i} className="flex items-start">
                      <span className="shrink-0 w-6 h-6 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center mt-0.5 me-3">
        ...
        <div className="mt-16 bg-linear-to-r from-violet-600 to-indigo-600 rounded-3xl p-8 sm:p-12 text-center text-white shadow-xl relative overflow-hidden">
          ...
          <h2 className="text-3xl font-bold mb-4 relative z-10">
            {isRTL
              ? "مستعد لإدارة مشاريعك باحترافية؟"
