...
import { recalculateTaskDates } from "./taskDates";
...
export async function createTask(taskData) {
  ...
  if (taskData.sectionAssignments && taskData.sectionAssignments.length > 0) {
    ...
    // No section assignments (either project has no sections, or simple assignment mode)
    flattenedAssignedTo = taskData.assignedTo || [];
  }
  ...
  if (Array.isArray(finalTaskData.dependsOn) && finalTaskData.dependsOn.length === 0) {
    finalTaskData.dependsOn = []; // Should be fine if schema is array
  } else if (!finalTaskData.dependsOn) {
    finalTaskData.dependsOn = [];
  }

  if (finalTaskData.dueDate === "") {
    finalTaskData.dueDate = null;
  }
  ...
  const task = await Task.create({ 
    ...
    sectionId: finalSectionAssignments.length > 0 ? finalSectionAssignments[0].sectionId : finalTaskData.sectionId,
    memberSubmissions 
  });
