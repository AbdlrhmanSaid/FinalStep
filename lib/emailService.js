...
// 6. Project Notification Alert (to All Members)
export const sendProjectNotificationEmail = async ({ emails, projectName, projectId, senderName }) => {
  const url = `${getBaseUrl()}/dashboard/projects/${projectId}`;
  await sendMail({
    to: emails,
    subject: `[تنبيه هام] تحديثات ومشاورات في مشروع: ${projectName}`,
    html: getTemplate(
      "تنبيه ومراجعة سريعة 🔔",
      `قام <strong>${senderName}</strong> بإرسال تنبيه عاجل لجميع أعضاء مشروع <strong>${projectName}</strong> للتحقق من المهام المطلوبة، ومراجعة آخر التحديثات الخاصة بالمشروع لضمان سير العمل في الوقت المحدد.`,
      url,
      "الدخول إلى المشروع الآن"
    ),
  });
};
